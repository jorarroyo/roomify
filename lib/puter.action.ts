...
export const createProject = async ({ item, visibility = 'private' }: CreateProjectParams):
    ...
    if (!PUTER_WORKER_URL) {
        console.warn('missing PUTER_WORKER_URL, skipping createProject');
        return null;
    }
    ...
    if (!resolvedSource) {
        console.error(`Failed to resolve source image URL: ${item.sourceImage}`);
        return null;
    }
...
export const getProjects = async () => {
    if (!PUTER_WORKER_URL) {
        console.warn('missing PUTER_WORKER_URL, skipping getProjects');
        return [];
    }

    try {
        const response
            = await puter.workers.exec(`${PUTER_WORKER_URL}/api/projects/list`, { method: 'GET' });
